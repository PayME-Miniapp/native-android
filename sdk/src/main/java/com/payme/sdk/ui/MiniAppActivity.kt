...
class MiniAppActivity : AppCompatActivity() {
  ...
  private fun startServer() {
    ...
    www_root = File("${filesDir.path}/www", "sdkWebapp3-main")
...
//    loadUrl = "http://10.8.103.29:3000/"
  ...
  private fun sendNativeDeviceInfo() {
    ...
    insets.put("top", Utils().pxToDp(this, statusHeight))
    val bottom = Utils().getRootWindowInsetsCompat(rootView!!) ?: 0
    insets.put("bottom", Utils().pxToDp(this, bottom.toInt()))
    val deviceInfo = JSONObject()

    deviceInfo.put("platform", "android")
    val deviceId = Settings.Secure.getString(this.contentResolver, Settings.Secure.ANDROID_ID)
    deviceInfo.put("deviceId", deviceId)
    deviceInfo.put("userAgent", Utils().getUserAgent(this))
    deviceInfo.put("version", packageManager.getPackageInfo(packageName, 0).versionName)
    deviceInfo.put("buildNumber", packageManager.getPackageInfo(packageName, 0).versionCode)
    deviceInfo.put("isEmulator", Utils().isEmulator())
    deviceInfo.put("brand", Build.BRAND)
    deviceInfo.put("model", Build.MODEL)
    deviceInfo.put("bundleId", packageName)
    deviceInfo.put("systemName", "Android")
    deviceInfo.put("systemVersion", Build.VERSION.RELEASE)
    deviceInfo.put("deviceType", DeviceTypeResolver(this).deviceType.value)

    deviceInfo.put("insets", insets)

    val biometric = JSONObject()
    biometric.put("isSupport", Utils().isBiometricReady(this))
  ...
  private fun downloadSourceWeb(url: String?, editor: SharedPreferences.Editor, patch: Int) {
    ...
    url?.let {
      Utils().download(this, it, sourceWeb.absolutePath) { totalBytesCopied, length ->
  ...
  @SuppressLint("SetJavaScriptEnabled", "CommitPrefEdits", "SetTextI18n")
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    versionCheckingTask = Thread {
      try {
        val versionFile = File(filesDir.path, "version.json")
...
//            .use { it.readText() }
        ...
        for (i in 0 until jsonArray.length()) {
          ...
          if (item.getString("version") == SDK_VERSION) {
            found = item
        ...
        val sharedPreference =
          getSharedPreferences("PAYME_NATIVE_UPDATE", Context.MODE_PRIVATE)
        ...
        runOnUiThread {
          textUpdateLabel.text = "Đang tải dữ liệu (${SDK_VERSION}.${patch})..."
        }
    ...
    updatingView = findViewById(R.id.updating_view)
    progressBar = findViewById(R.id.progress)
    textProgress = findViewById(R.id.progress_text)
    textUpdateLabel = findViewById(R.id.update_label_text)
    lottieView = findViewById(R.id.lottieView)
    lottieContainerView = findViewById(R.id.lottie_container_view)
    ...
    rootView!!.viewTreeObserver.addOnGlobalLayoutListener {
      ...
      val height = Utils().pxToDp(this, heightDiff)
      ...
      if (heightDiff > 100) {
        ...
        Utils().evaluateJSWebView(this, myWebView!!, "nativeKeyboardHeight", "0", null)
      }
    }
    ...
    myWebView?.apply {
      ...
      webViewClient = object : WebViewClient() {
        ...
        override fun onPageFinished(view: WebView?, url: String?) {
          ...
          if (url == loadUrl) {
            ...
            Utils().sendNativePref(this@MiniAppActivity, myWebView!!)
          }
        }
      ...
      val javaScriptInterface = JavaScriptInterface(
        ...
        sendNativePreferences = { Utils().sendNativePref(this@MiniAppActivity, this) },
        ...
        openSettings = { PermissionCameraUtil().openSetting(this@MiniAppActivity) },
    ...
    payMEUpdatePatchViewModel.getDoneUpdate().observe(this) {
      if (it) {
        unzip()
        startServer()
    ...
    payMEUpdatePatchViewModel.getShowUpdatingUI().observe(this) {
      runOnUiThread {
        if (it) {
    ...
    payMEUpdatePatchViewModel.getIsLostConnection().observe(this) {
      if (!it) {
    ...
    notificationViewModel.getNotificationData().observe(this) {
      if (it.length() != 0) {
  ...
  private fun openWebView(data: String) {
    try {
      ...
      if (content.isNotEmpty() && type.isNotEmpty()) {
        ...
        subWebView.show(supportFragmentManager, "SUBWEBVIEW")
      }
    } catch (e: Exception) {
  ...
  private val requestContactsPermissionLauncher =
    registerForActivityResult(
      ...
      if (isGranted) {
        Utils().nativePermissionStatus(this, myWebView!!, "READ_CONTACTS", "GRANTED")
        Utils().getContacts(this, myWebView!!)
      } else {
        Utils().nativePermissionStatus(this, myWebView!!, "READ_CONTACTS", "DENIED")
      }
    }
    ...
    try {
      when {
        ...
        ActivityCompat.shouldShowRequestPermissionRationale(
          ...
          Utils().nativePermissionStatus(this, myWebView!!, "READ_CONTACTS", "BLOCKED")
        }
        else -> {
  ...
  private val requestPermissionLauncher =
    registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted: Boolean ->
      ...
      if (isGranted) {
        Utils().nativePermissionStatus(this, myWebView!!, permissionType, "GRANTED")
      } else {
        Utils().nativePermissionStatus(this, myWebView!!, permissionType, "DENIED")
      }
      permissionType = ""
    }
    ...
    try {
      ...
      when {
        ...
        ActivityCompat.shouldShowRequestPermissionRationale(
          ...
          Utils().nativePermissionStatus(this, myWebView!!, permissionType, "BLOCKED")
        }
        else -> {
    ...
    try {
      ...
      when {
        ContextCompat.checkSelfPermission(
          ...
          Utils().nativePermissionStatus(this, myWebView!!, "CAMERA", "GRANTED")
        ...
        ActivityCompat.shouldShowRequestPermissionRationale(
          ...
          Utils().nativePermissionStatus(this, myWebView!!, "CAMERA", "BLOCKED")
        }
        else -> {
  ...
  private val requestCardKycPermissionLauncher =
    registerForActivityResult(
      ...
      if (isGranted) {
        ...
        Utils().nativePermissionStatus(this, myWebView!!, "CAMERA", "DENIED")
      }
    }
  ...
  private fun startFaceDetectorActivity(data: JSONObject) {
    ...
    val intent = Intent(this, com.payme.sdk.ui.FaceDetectorActivity::class.java)
    intent.putExtra("title", title)
  ...
  private val evaluateJsDataObserver: Observer<Pair<String, String>> = Observer {
    if (it.first.isNotEmpty() && myWebView != null) {
      Utils().evaluateJSWebView(this@MiniAppActivity, myWebView!!, it.first, it.second, null)
    }
  }
  ...
  override fun onResume() {
    ...
    Utils().evaluateJSWebView(this, myWebView!!, "nativeAppState", "\"active\"", null)
  }

  override fun onPause() {
    super.onPause()
    nativeAppState = "background"
    Utils().evaluateJSWebView(this, myWebView!!, "nativeAppState", "\"background\"", null)
  }
